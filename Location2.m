function Location2(currentLabel)
...
if clientID > -1
    disp(['Connected to CoppeliaSim for path execution to drop ', currentLabel]);
    ...
    % goalLocation = [0,-1.5, -pi/2]; % 

    % Define goal location based on object label
    switch currentLabel
        case 'bottle'
            goalLocation = [-0.1, -1.5, -pi/2];
        case 'orange'
            goalLocation = [0, -1.5, -pi/2];
        case 'cup'
            goalLocation = [0.1, -1.5, -pi/2];
    ...
    executePathPID2(clientID, sim, path, pioneerHandle, leftMotorHandle, rightMotorHandle);
...
% Release Remote API resources (originally commented out)
% sim.delete();
