function Cartesian_grasping_script1(label, objectData)
...
% Determine object position based on passed objectData
rframex = str2double(objectData(2));
rframey = str2double(objectData(3));
rframez = str2double(objectData(4));

if strcmp(label, 'bottle')
    rframey = rframey - 0.01;
end
...
waypoints = [
    ...
    -rframex, -rframey, rframez + 0.28, 0, 180, 0;
    -rframex, -rframey, rframez + 0.24, 0, 180, 0;
    ];
...
if clientID > -1
    ...
    for seg = 1:numSegments
        ...
        % Plot the joint trajectories for the current segment
        figure(3);
        for j = 1:dof
            subplot(3,2,j);
